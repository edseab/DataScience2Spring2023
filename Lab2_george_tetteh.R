...
prequel_matrix
rownames(prequel_matrix)<-c('phantom_menace','attack_of_clones','revenge_of_sith')
...
# this few Water signals (or fewer).
  my_binomial_cdf<-function(n,p,k){
    
    prob= pbinom(k,n,p)
    return(prob)
  }
  my_binomial_cdf(20,0.7,11)
...
# What do you conclude to the astronomer?
 rbinomvalues<- rbinom(100000,20,0.7)
hist(rbinomvalues)
prob=  sum(rbinomvalues<=11)/100000
prob
...
# because it is a two tail t-test
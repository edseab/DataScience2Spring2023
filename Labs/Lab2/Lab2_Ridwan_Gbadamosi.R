...
box_office_matrix <- matrix(box_office, byrow = T, nrow = 3)

#-------------- Naming rows and columns ---------------#
rownames(box_office_matrix) <- c("The Phantom Menace", "Attack of clones", "Revenge of sith")
colnames(box_office_matrix) <- c("US revenue", "International revenue")

#-------------------Binding the third column to the matrix----------------#
box_office_matrix <- cbind(box_office_matrix, rowSums(box_office_matrix))

#------------------Appending the new matrix to the main matrix table------#
star_wars_matrix <- rbind(star_wars_matrix, box_office_matrix)
star_wars_matrix
...
# The str function gives details and different information about the data frame
...
binomial_dist(10, 8, 0.7)

# compare this to dbinom(8,10,0.7)
dbinom(8, 10, 0.7)
# The two functions produce the same result.
...
# Simulate 100,000 universes where the aliens sent out 20 probes to Earth
probes <- rbinom(n = n_simulations, size = 20, prob = 0.7)

# Calculate the proportion of universes where the number of probes signaling water is 11 or fewer
prop <- mean(probes <= 11)

# Print the proportion
prop

#The output is approximately 0.11351, or 11.4%. In other words, if the planet in question is Earth,
#there is only a 11.4% chance of observing 11 or fewer water signals out of 20 probes.

#Therefore, based on these simulations and calculations, we can conclude that it is quite unlikely that 
#the planet in question is Earth, given that only 11 out of 20 probes signaled water.
...
# Compare this to:
qnorm(0.1,175, 10, lower.tail=FALSE) #It is he same with the upper 10th percentile
...
#Both matrices output the third column (Total revenue) of each observation.

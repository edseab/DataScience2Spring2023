...
box_office_2 <- c(phantom_menace, attack_of_clones, revenge_of_sith)
movie_matrix <- matrix(box_office_2, byrow = T, nrow = 3)
movie_matrix

rownames(movie_matrix) <- c("phantom_me", "attack_of_clones", "revenue_of_sith")
colnames(movie_matrix) <- c("US revenue", "International revenue")
total_revenue <-rowSums(movie_matrix)
total_revenue
movie_matrix <- cbind(movie_matrix,total_revenue)
movie_matrix
star_wars_matrix <- rbind(star_wars_matrix, movie_matrix)
star_wars_matrix
...
star_wars_matrix["phantom_me", "Total revenue"]
...
binomial_function <- function(p,n,k){
  q <- 1-p
  w <- n-k
  calculated_val <- choose(n,k) *(p^k)*(q^w)
  
  return (calculated_val)
}
binomial_function(0.7,3,1)
binomial_function(0.7,10,8)
...
# and less than 60% of the probes signaled water
...
binomial_cdf <- function(p, n, k) {
  q <- 1 - p
  cdf <- pbinom(k, size = n, prob = p)
  return(cdf)
}
binomial_cdf(0.7,20,11)
...
alien_simulation <- rbinom(100000, 20, 0.7)
prob <- sum(alien_simulation <= 11)/num_simulation
prob
...
# The multiplication by 2 was to account for the two directions of the test
...
rownames(prequel_trilogy_matrix) <- c("The Phantom Menace","Attack of clones","Revenge of sith")
total_revenue <- rowSums(prequel_trilogy_matrix)
prequel_trilogy_matrix <- cbind(prequel_trilogy_matrix, total_revenue)
colnames(prequel_trilogy_matrix)[3] <- "Total revenue"
...
my_list$boolean <- star_wars_matrix
...
num_water_probes <- rbinom(num_universes, 20, 0.7) 
prop <- sum(num_water_probes <= 11) / num_universes
prop

#MY CONCLUSION
#Based on the scenario, we calculate that the likelihood of receiving 11 or less water signals out of 20 probes is roughly 0.0545 or 5.45%, 
#if this planet is truly Earth.It is highly unlikely that this planet is Earth 
#based on the predicted likelihood of receiving 11 or less water signs out of 20 probes.
...
#While performing a two-tailed t-test, we multiply the p-value by 2 to determine whether the means of the two samples differ substantially 
#from one another in either direction (i.e., whether one is larger than the other, or whether one is smaller than the other).

...
new_vector <- c(phantom_menace, attack_of_clones, revenge_of_sith)
new_matrix <- matrix(new_vector, byrow = T, nrow = 3)
rownames(new_matrix) <- c("The Phantom Menace", "Attack of Clones", "Revenge of Sith")
...
star_wars_matrix[4,2] # this is for the international revenue of The Phanthom Menace
star_wars_matrix[4,3] # this is for the international revenue of The Phanthom Menace
...
star_wars_matrix[,2] # outputs the international revenue for all movies
star_wars_matrix[4,] # outputs the national, international, and total revenue for the fourth movie
...
my_binom_fct <- function(n,k,p) (p^k) * ((1-p)^(n-k)) * choose(n,k)
...
set.seed(123)
simu_E_W <- rbinom(100000, 20, 0.7)
simu_E_W
# and calculate in what percentage of these universes the number of probes signalling Water is 11 or fewer
percent_simu_E_W_lesseq11 <- sum(simu_E_A <= 11)/100000
percent_simu_E_W_lesseq11
# What do you conclude to the astronomer?
'Conclusion: The percentage of these universes where the number of probes signaling Water in 11 or fewer probes is not equal to 70%: therefore, the astronomer was correct about the probes not being sent to Earth but the likelihood he predicted is actually much less than 60% after 100,000 simulations.'
...
qnorm(0.9, 175, 10)
# Compare this to:
qnorm(0.1,175, 10, lower.tail=FALSE) # same as previous one: qnorm(0.9, 175, 10)
...
'We multiplied by 2 because this is a two-tailed test.'
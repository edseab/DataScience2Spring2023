...
movies_matrix <- matrix(box_office, byrow = T, nrow = 3)
movies_matrix
rownames(movies_matrix) <- c("phantom_menace", "attack_of_clones", "revenge_of_sith")
movies_matrix
total_revenue <- rowSums(movies_matrix)
movies_matrix <- cbind(movies_matrix, total_revenue)
movies_matrix

#bind
star_wars_matrix <- rbind(movies_matrix, star_wars_matrix)
star_wars_matrix
colnames(star_wars_matrix) <- c("US revenue", "International revenue", "Total_revenue")
star_wars_matrix
...
star_wars_matrix["phantom_menace", "Total_revenue"]
...
# out of n attempts each with probability p of success,
...
k <- 8    # number of probes that send a signal of water
p <- 0.7  # probability of water for each probe

# calculate the probability using the binomial_prob() function
...
# Define the parameters
n <- 20   # number of probes sent out
k <- 11   # number of probes signalling water
p <- 0.7  # probability of water on Earth

# Simulate 100,000 universes
set.seed(123)  # set the seed for reproducibility
num_sims <- 100000
num_water <- rbinom(num_sims, n, p)

# Calculate the proportion of universes where we get 11 or fewer water signals
prop_water_11_or_fewer <- sum(num_water <= 11) / num_sims

# Print the result
prop_water_11_or_fewer
# What do you conclude to the astronomer?
#This suggests that if the planet being observed is Earth, the probability of 
#getting 11 or fewer water signals out of 20 probes is quite low (less than 0.05%).
#Therefore, the astronomer may conclude that it is unlikely that the planet being
#observed is Earth, given the observed data.
...
### 4.3
# Let's compare the box office returns of the of and prequel trilogies
...
my_t <- function(x1, x2) {
  ...
  # Calculate degrees of freedom using Welch-Satterthwaite formula
  df <- (s1/n1 + s2/n2)^2 / ((s1/n1)^2/(n1-1) + (s2/n2)^2/(n2-1))
  
  # Calculate two-tailed p-value using cumulative density function
  p_value <- 2 * pt(abs(t_stat), df)
  
  # Return a list with the t-statistic, degrees of freedom, and p-value

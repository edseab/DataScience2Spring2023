...
box_office_matrix <- c(phantom_menace, attack_of_clones, revenge_of_sith)
box_office_matrix <- matrix(c(phantom_menace, attack_of_clones, revenge_of_sith), byrow = TRUE, nrow = 3)
rownames(box_office_matrix) <- c("phantom_menace", "attack_of_clones", "revenge_of_sith")
...
star_wars_matrix <- rbind(my_list$matrix, box_office_matrix)
...
#p= 0.1133315, this greater than 0.05 so we fail to reject null hypothesis and conclude that it is likely that the probes were sent to Earth.
...
# So, we need to multiply by 2 to show the direction of the test since it is two tail.

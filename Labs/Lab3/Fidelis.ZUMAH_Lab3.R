...
selected_rows <- mtcars[mtcars$mpg >= 25 | mtcars$wt <= 2.5, ]
selected_rows
...
probe <- function(n, w) {
  result <- n * w + sqrt(n^2 + w^2)
  return(result)
}

# The function should return a character vectopr of length n, consisting of 'Water' and 'Land', sampled with probability w. (so probability of sampling 'Water' is w)
...
probe <- function(n, w) {
  ...
  sample_vec <- sample(c("Water", "Land"), n, replace = TRUE, prob = c(w, 1 - w))
  return(sample_vec)
}
...
# It (37.2851) represents the predicted value of the dependent variable (mpg) when the independent variable (wt) is equal to zero.
### 5.2

# What does the Estimate for the mtcars$wt number represent?
#it represents the slope  which is expected change in the dependent variable (mpg) for every unit increase in the independent variable (wt), holding all other variables constant.

### 5.3 
# Is the relationship between these two variables positive or negative? Why do you think that might be?
#The relationship between the two variables, mpg and wt, is negative. 
#This can be inferred from the negative coefficient for mtcars$wt, which is -5.3445. 
#This means that as the weight of a car (measured in 1000 pounds/500 kg) increases, the miles per gallon (mpg) decreases.
...
var(mtcars$wt_centred) #0.957379
var(mtcars$wt) #0.957379
mean(mtcars$wt_centred) #3.469447e-17
mean(mtcars$wt) #3.21725
#the mean is changes but the variance 0.957379 remain same 
# 
### 5.6
# Run a new regression with new independent variable
premodel <- lm(mtcars$mpg ~ mtcars$wt_centred)
summary(premodel)
# What do you notice about the estimates?
# What is the interpretation of the (Intercept) estimate in this regression?
#the intercept changes but the slope remains. 
#when the dependent variable(mpg) is 20.0906  when the independent variable (wt) is equal to the mean.
...
estimate
#I got the same two estimate values I got when I ran the first model of regression

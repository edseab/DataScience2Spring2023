...
probe <- function(n,w){
  if(n<0 & n>1) print('please input a probability between 1 and 0')
  if(class(w) != "numeric") print('input a numeric character for w')
  final <- print(paste('Water and Land with probability', n))
  return(final)
}
probe(0.8,5)
w <- 6
...
strsplit('Hello to you too. /My name is Arinze.', split='/')
...
# it represent the intercept value of miles per gallon when other variables are kept constant
### 5.2
...
# The variance of the transformed and the untransformed are the same while the means are different
### 5.6
...
# The estimate coefficients have changed when compared with the previous estimates of the untransformed

# What is the interpretation of the (Intercept) estimate in this regression?
# The intercept 20.0906 indicates the value of miles per gallon when other variables are kept constant
### 5.7
...
x
solve(t(x)%*%x) %*% (t(x)%*%y)
#The result from the code was the coefficient estimate of the linear regression for the miles per gallon and wt

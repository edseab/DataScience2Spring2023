...
mtcars[(mtcars$cyl >= 6)&(mtcars$hp >= 110),] # or the line below 
mtcars[(mtcars$cyl >= 6)&(mtcars$hp >= 110),1:ncol(mtcars)]
...
probe <- function(n,w){
  if (!is.numeric(w) ){return("Please input a probability between 0 and 1")
  }
  
  if (w < 0 | w > 1 ){return("Please input a probability between 0 and 1")
  }
  
  sample(c("Water", "Land"), n, replace=TRUE, prob=c(w, 1-w))
...
prodd <- 1
max_num <- 0
while(prodd<= 10000000){
  max_num = max_num + 1
  prodd = prodd * (max_num)
}
print(max_num)

# We need 11 first numbers
...
# Intercept represents the value of miles per gallon when the weight is equal to 0

### 5.2
# What does the Estimate for the mtcars$wt number represent?

# The estimate for mtcars$wt represents the slope. 
# It's the number of units how the miles per gallon increase when the weight increases for 1 unit
...
est <- -5.3445*((4000/2000)/(1000/500)) + 37.2851
print(est)
# 31.9406
...
# The transformed variable has a mean equal to 0 because it has been centered
var(mtcars$wt)
var(mtcars$wt_centred)
# But both two variables has the same variance
...
# We realize that the intercept has changed but the slope has not.
# The new intercept gotten can be interpreted as the mile per gallon of a car 
# which has a weight equal to the average weight of the sample 
...
# I found exactly the two estimates value we got when we ran the first model of regression

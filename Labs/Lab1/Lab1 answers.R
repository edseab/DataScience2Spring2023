...
# f2 returns TRUE if the 1st element is cleanly divisible by the second element, FALSE otherwise
...
one.die.roll <- function(x)sample(1:6,x, replace=T)
two.dice.roll <- function(x) one.die.roll(x) + one.die.roll(x)
...
hist(two.dice.roll,10,breaks=1:12)
hist(two.dice.roll,50,breaks=1:12)
hist(two.dice.roll,100,breaks=1:12)
hist(two.dice.roll,1000,breaks=1:12)

# The more rolls we simulate the closer the histogram approximates the probability mass function of the double dice roll as calculated in class.
...
# The total area under the curve of a pdf must equal 1. The area under a flat line can be calculated as the area of a rectangle with width equal to the difference between the upper and lower bounds, and the height equal to the density. We can therefore solve for the density:

# density = 1/(upper-lower)

# for a pdf bounded between 0 and 1 the density must be 1/(1-0) = 1
...
#The answer is the same: density = 1/(6-5) = 1
...
# Based on the results of this code and your answers above, what can you conclude about the purpose of the dunif(x,min,max) function?

# dunif(x,min,max) returns the value of the probability density at value x for a uniform pdf bounded between min and max.

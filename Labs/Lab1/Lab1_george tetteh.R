...
odd_digits <- function(digits) digits[ digits %% 2 == 1 ]
odd_digits(digits)
...
# F2 is to verify whether x is a multiple of y or whether y is a factor of x
...
Sum_roll<- function(x){
TheSum=sample(1:6, size =x, replace = TRUE)+sample(1:6, size = x, replace = TRUE)
return(TheSum)
}
Sum_roll(5)
### 4.2
...
for (n in n_rolls){
  rolls<-replicate(n,Sum_roll())
  hist(rolls,breaks=1:12,main=paste("Histogram of",n ,"double dice roll"))
}
...
# The probability density of a  uniform PDF  for values is equal to 1 thus 1/(1-0)
### 4.5
...
# The probability density is equal to 1
### 4.6
...
# The probability density is 2. Thus  1/(0.5-0)
### 4.7
...
#The probability density is 0.5.  Thus 1/(2-0)
### 4.8
...
# The dunif function is used to calculate the PDF

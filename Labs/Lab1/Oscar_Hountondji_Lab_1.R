...
#f2 checks if x is a multiple of y by returning yes or no (yes is the remainder is equal to 0 and no if not)


### 3.2
# Based on the definition of the mean from today's lecture, write a function that calculates the mean of all of the elements of a vector. assign it to the object my.mean. You will find the functions 'sum' and 'length' useful here.
my.mean <- function(V){
  the_mean =  sum(V)/length(V)
  return(the_mean)
}
...
roll_sum <- function(x){
  the_sum = sample(1:6, x, replace = TRUE) + sample(1:6, x, replace = TRUE)
  return(the_sum)
}
#example :
roll_sum(4)
...
#Prof, What do you mean by "the results of double dice rolls" ? Are you talking about the sum
res_10 = roll_sum(10)
dev.new()
hist(res_10, breaks=1:12)

res_100 = roll_sum(100)
dev.new()
hist(res_100, breaks=1:12)

res_1000 = roll_sum(1000)
dev.new()
hist(res_1000, breaks=1:12)

res_10000 = roll_sum(10000)
dev.new()
hist(res_10000, breaks=1:12)

#Comment : the more we roll the dice, more normal and symetric is the distribution
...
# For any x between 0 and 1, U(x) = 1/(1-0) = 1 since the sum of a  probability density is equal to 1, the uniform pdf assumes that all the possibilities have the same probability  and in our case the uniform pdf is bounded between 0 and 1
...
#For any x between 5 and 6, U(x) = 1/(6-5) = 1 and U(x) = 0 otherwise
...
#For any x between 0 and 0.5, U(x) = 1/(0.5-0) = 2 and U(x) = 0 otherwise
...
#For any x between 0 and 2, U(x) = 1/(2-0) = 0.5 and U(x) = 0 otherwise
...
# the dunif function is used to get the uniform distribution
# the dunif function can be used to calculate the probability density for any range of values within that interval

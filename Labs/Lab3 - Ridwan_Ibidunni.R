...
#This line of code prints the rows with at least 6 cylinders and at least 110 hoursepower conditions
...
#This line of code prints the rows with at least 25 miles per gallon and the maximum weight of 2.5 conditions
...
# This represents the intercept on the y axis of the model

### 5.2
# What does the Estimate for the mtcars$wt number represent?
#This represents the value of the independent variable (weight) of the model
#This value will determine the dependent variable
...
#using the model we have -10650.715
...
summary(model)
#I observed that this still produced the same slope at the first model
#but the estimated intercept is 20.0906
...
as.vector((t(x) %*% x )^(-1)) * (t(x) * y)

#It will be widely observed that most of the output is less than 1